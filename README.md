...
划译是 Windows 托盘小工具：在任意软件里**选中中文**，按 `Ctrl + Shift + A`，选区会被替换成地道英文。DeepSeek 负责翻译，你继续用母语组织内容即可。
...
- **DeepSeek 精准翻译**：对接 DeepSeek 对话模型，商务、口语、礼貌用语都更贴语境。
...
| [**v0.1.2**](https://github.com/jiangchuan-syber/talk-with-the-world/releases/tag/v0.1.2) | 划译 0.1.2，修复全局快捷键可靠性，x64 安装包 `划译_0.1.2_x64-setup.exe` |
...
直接下载 v0.1.2 安装包（Release 构建完成后可用）：

<https://github.com/jiangchuan-syber/talk-with-the-world/releases/download/v0.1.2/%E5%88%92%E8%AF%91_0.1.2_x64-setup.exe>

## 功能

- **全局快捷键**：选中中文后按 `Ctrl + Shift + A`，自动复制、翻译、粘贴英文结果
- **剪贴板友好**：翻译完成后会尝试恢复翻译前的剪贴板文本
- **系统托盘**：无前台主窗口，通过托盘图标打开设置或暂停/启用
- **可配置 API**：填写 DeepSeek API 密钥即可使用，支持选择翻译模型
...
产物位于 `src-tauri/target/release/bundle/nsis/`，例如 `划译_0.1.2_x64-setup.exe`。安装包对外名称为 **划译**（`productName`）；当前仅生成 NSIS 安装包（中文产品名下 MSI 易失败）。

发布新版本：推送 `v*` 标签（如 `v0.1.2`）会触发 [`.github/workflows/release.yml`](.github/workflows/release.yml) 自动构建并上传到 [Releases](https://github.com/jiangchuan-syber/talk-with-the-world/releases)。

## 使用方法

1. 首次运行后，从系统托盘打开 **设置**，填写 API Key，并按需选择翻译模型，点击保存。
2. 在 Word、浏览器、编辑器等任意窗口中**选中一段中文**。
3. 按下 **`Ctrl + Shift + A`**，等待翻译完成；选区会被替换为英文译文。
...
翻译请求固定发往 DeepSeek 官方接口，无需手动配置地址。
